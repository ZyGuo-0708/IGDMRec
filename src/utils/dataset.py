...
class RecDataset(object):
    # RecDataset类有两个属性：config df，以及数个方法（后面的函数）；实例化：如dataset = RecDataset(config) 将config赋值给config
    def __init__(self, config, df=None):
        ...
        # load rating file from data path? #self df 加载数据
    ...
    def load_inter_graph(self, file_name): #RecDataset.load_inter_graph()调用方法
        inter_file = os.path.join(self.dataset_path, file_name) #数据集地址
        cols = [self.uid_field, self.iid_field, self.splitting_label]
        self.df = pd.read_csv(inter_file, usecols=cols, sep=self.config['field_separator']) # self.df
        if not self.df.columns.isin(cols).all():
            raise ValueError('File {} lost some required columns.'.format(inter_file))

    def split(self): #数据集划分
        dfs = []
