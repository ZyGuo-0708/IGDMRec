...
class AbstractDataLoader(object):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        self.sparsity = 1 - self.dataset.inter_num / self.dataset.user_num / self.dataset.item_num # 计算数据集稀疏度
        self.pr = 0        # 数据加载器指针
        self.inter_pr = 0  # 交互指针
...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        if self.neighborhood_loss_required: #领域损失 NONE
    ...
    def pretrain_setup(self): #预处理 打乱物品ID序列
        """
        Reset dataloader. Outputing the same positive & negative samples with each training.
        :return:
        """
        # sort & random
        if self.shuffle: #Flse
            self.dataset = self.dataset_bk.copy(self.dataset_bk.df)
        self.all_items.sort() # item ID 排序
        if self.use_full_sampling: # 使用全采样：False； 如果使用则对Uid排序
            self.all_uids.sort()
        random.shuffle(self.all_items) #打乱物品id序列
        print("OK")
    ...
    def inter_matrix(self, form='coo', value_field=None):
        ...
        return self._create_sparse_matrix(self.dataset.df, self.dataset.uid_field,
                                          self.dataset.iid_field, form, value_field)   #uid_field : uid字段：userID #iid_field: itemID

    def _create_sparse_matrix(self, df_feat, source_field, target_field, form='coo', value_field=None): #创建稀疏交互矩阵
        """Get sparse matrix that describe relations between two fields.

        Source and target should be token-like fields. ####source: userID ; target: itemID
        ...
        if value_field is None: #df(dataframe): 118551*1
        ...
        mat = coo_matrix((data, (src, tgt)), shape=(self.dataset.user_num, self.dataset.item_num)) #建立U-I矩阵
