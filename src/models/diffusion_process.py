...
class DiffusionProcess(nn.Module):
    def __init__(self,config, noise_schedule, noise_scale, noise_min, noise_max,steps, device, keep_num=10):
    ...
    def diffusion_setting(self):
        ...
        self.alphas_cumprod = th.cumprod(alphas, axis=0).to(self.device) #\bar{alpha}
        self.alphas_cumprod_prev = th.cat([th.tensor([1.0]).to(self.device), self.alphas_cumprod[:-1]]).to(self.device)  # \bar{alpha}_{t-1}
        self.alphas_cumprod_next = th.cat([self.alphas_cumprod[1:], th.tensor([0.0]).to(self.device)]).to(self.device)  # \bar{alpha}_{t+1}
        assert self.alphas_cumprod_prev.shape == (self.steps,)

        self.sqrt_alphas_cumprod = th.sqrt(self.alphas_cumprod) #\sqrt{\bar{\alpha}}
        self.sqrt_one_minus_alphas_cumprod = th.sqrt(1.0 - self.alphas_cumprod)   #\sqrt{ 1 - \bar{\alpha} }
        self.log_one_minus_alphas_cumprod = th.log(1.0 - self.alphas_cumprod)     #\log { 1 - \bar{\alpha} }
        self.sqrt_recip_alphas_cumprod = th.sqrt(1.0 / self.alphas_cumprod)       #\sqrt {1/\bar{\alpha}}
        self.sqrt_recipm1_alphas_cumprod = th.sqrt(1.0 / self.alphas_cumprod - 1) #\sqrt { 1/\bar{\alpha} -1 }

        # self.posterior_variance = (
        #     self.beta_nums * (1.0 - self.alphas_cumprod_prev) / (1.0 - self.alphas_cumprod)   #\beta_t * (1-\bar{alpha_{t-1}}) / (1-bar{\alpha_t})
        # )
        self.posterior_variance = self.beta_nums
        ...
        self.posterior_mean_coef1 = (
            self.beta_nums * th.sqrt(self.alphas_cumprod_prev) / (1.0 - self.alphas_cumprod) # \beta_t * (\sqrt\alpha_{t-1}) / (1-\bar{\alpha}_t)
        )
    ...
    # calculate loss 训练
    def caculate_losses(self, model, emb_s, con, reweight=False):
        batch_size, device = emb_s.size(0), emb_s.device
        ts, pt = self.sample_timesteps(batch_size, device, 'uniform')
        noise = th.randn_like(emb_s) #噪声取样 N(0,1)
        emb_t = self.forward_process(emb_s, ts, noise) # x_0 ——> x_t
        terms = {}
        rand_value = th.rand(1).item()

        if rand_value < 0.1: ### p_{\mu}
            model_output = model.forward_unco(emb_t, ts)
        else:
            model_output = model(emb_t, con, ts)


        assert model_output.shape == emb_s.shape

        mse = mean_flat((emb_s - model_output) ** 2) #MSE LOSS
    ...
    def p_sample(self, model, emb_s, con, steps, sampling_noise=False):
        assert steps <= self.steps, "Too much steps in inference."
        # if steps == 0:
        emb_t = emb_s
        # else:
        #     t = th.tensor([steps - 1] * emb_s.shape[0]).to(emb_s.device)
        #     #mb_t = self.q_sample(emb_s, t)
        #     emb_t = self.forward_process(emb_s, t, noise=None)  #x_0 ——> x_t
        indices = list(range(self.steps))[::-1]

        #Gaussian
        # emb_t = th.randn_like(emb_s)

        if self.noise_scale == 0.: #noise_scale = 1
            for i in indices:
                t = th.tensor([i] * emb_t.shape[0]).to(emb_s.device)
                emb_t = model.forward(emb_t,con, t)
            return emb_t

        for i in indices:
            t = th.tensor([i] * emb_t.shape[0]).to(emb_s.device)
            out = self.p_mean_variance(model, emb_t, con, t)  # x_{t-1}
            if sampling_noise:
    ...
    def forward_process(self, emb_s, t, noise=None):
        ...
        return (
                self._extract_into_tensor(self.sqrt_alphas_cumprod, t, emb_s.shape) * emb_s     #input: sqrt{\bar{\alpha_t}} ,t ,x_0.shape
                + self._extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, emb_s.shape) #\sqrt{ 1 - \bar{\alpha} }
                * noise
        )
    ...
    def p_mean_variance(self, model, x, con, t):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output = model(x, con, t) #^x_0
        model_output_unco = model.forward_unco(x, t)

        model_variance = self.posterior_variance #\beta_t
        model_log_variance = self.posterior_log_variance_clipped #log \beta_t

        model_variance = self._extract_into_tensor(model_variance, t, x.shape)
        model_log_variance = self._extract_into_tensor(model_log_variance, t, x.shape)
        w = self.w
        pred_xstart = (1+w) * model_output -w * model_output_unco #1.1 1.1*con - 0.1*uncon
        model_mean, _, _ = self.q_posterior_mean_variance(emb_s=pred_xstart, emb_t=x, t=t) #mean
    ...
    def SNR(self, t):
        ...
        return self.alphas_cumprod[t] / (1 - self.alphas_cumprod[t])  # \bar{\alpha_t} / (1 - \bar{\alpha_t})
